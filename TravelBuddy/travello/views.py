...
def travello(request, username):
    ...
    # CHECK IS USER AUTHENTICATED(i.e, is the user logged in or not)
    # EXECUTE "if block" IF USER AUTHENTICATED
    ...
    # EXECUTE "else block" IF USER IS NOT AUTHENTICATED
    else:
...
def destination_details(request, username, destination_id):
    ...
    if request.user.is_authenticated:
        ...
        return render(request, 'destinations.html', {'dest': dest, 'dests_local': dests_local, 'guides': guides,
                                                     'hotels': hotels, 'destination_id': destination_id,
                                                     'vacancy': vacancy, 'can_be_booked': can_be_booked,
                                                     'general_details': general_details, 'hotel_images': hotel_images})
...
def bookings(request, username, destination_id):
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            # REDIRECTING THE USER BACK TO THE CORRESPONDING "destination page"
            return redirect('/travello/TravelBuddy/' + username + '/' + destination_id)
